...
export class Ticket extends BaseEntity {
  constructor(
    ...
    this.issuer_department_id = issuer_department?.id;
  ...
  @Column({
    type: "enum",
    enum: TicketTypeEnum,
    default: TicketTypeEnum.SERVICE_REQUEST,
    nullable: false,
  })
  type: TicketTypeEnum;
  ...
  async generate_ticket_open_history_entry() {
    ...
    await TicketHistoryAction.addToHistory(
      history,
      TicketHistoryActionEnum.OPEN,
      this.type,
      false
    );
  }
  ...
  private async define_last_ticket() {
    ...
    if (
      last_ticket.responsible_department_id != this.responsible_department_id
    ) {
      this.previous_responsible_department_id =
        last_ticket.responsible_department_id;
    }
  }
  ...
  private async add_history_action_entries() {
    ...
    const actions = await TicketHistoryAction.find({
    ...
    if (actions.length >= 1) {
      ...
      switch (action.type) {
        ...
        case TicketHistoryActionEnum.GROUP_REMOVE:
          const ticket = await Ticket.findOne({
            where: { owner_group_id: this.group_id },
          });
      ...
      if (this.short_description != last_ticket.short_description) {
        changed = true;
        await this.add_change_action_to_history(
          history,
          TicketChangeParamEnum.SHORT_DESCRIPTION,
          last_ticket.short_description,
      ...
      if (this.description != last_ticket.description) {
        ...
        await this.add_change_action_to_history(
          history,
          TicketChangeParamEnum.DESCRIPTION,
          last_ticket.description,
          this.description
        );
      }
      ...
      if (this.service_id != last_ticket.service_id) {
        ...
        await this.add_change_action_to_history(
          history,
          TicketChangeParamEnum.SERVICE,
          service.service_id,
          last_service.service_id
        ); // Todo: Add name to service
      }
    }
  }

  private async add_change_action_to_history(
