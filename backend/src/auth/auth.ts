...
export const SECRET_CHARS =
  "0123456789abcdefghijklmnopqrstuvwxyz!@#$%^&*()[]{}/+-_:.;,£àÀÉéÈè°ç\"<>`?'´=¬|¢~ABCDEFGHIJKLMNOPQRSTUVWXYZ";

export const encrypt_private_key = (key: string): string => {
  const cipher = crypto.createCipheriv(
    ENCRYPTION_ALGORITHM,
    process.env.PRIVATE_KEY_HASH_KEY,
    process.env.PRIVATE_KEY_CRYPTO_KEY
  );
...
export const verify_password = (
  ...
  return hash_password(to_check) == original;
};
