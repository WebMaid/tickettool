...
export const UserSettingTokenIndexPage: React.FC<Props> = () => {
  const { data, loading } = useGetKeysOfUserQuery({
  ...
  const createNewApiKey = async (note: string): Promise<void> => {
    ...
    try {
      const response: GenerateApiKeyResponse = await (
        await fetch(settings.backend.generate_api_key.url, {
  ...
  return (
    <div className="user-settings-developer">
