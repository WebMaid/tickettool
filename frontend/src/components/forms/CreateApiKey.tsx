...
import {
  generateScopeCategoryFromApiResponse,
  ScopeCategory,
} from "../../objects/ScopeCategory";
...
export const CreateApiKeyFormComponent: React.FC<Props> = ({
  submit,
  scopeError,
}) => {
  ...
  const {
    ...
    control,
    formState: { errors },
  } = useForm<IApiKeyCreateFormInputs>({
    defaultValues: {
    ...
    <form onSubmit={handleSubmit(submit)}>
      <div>
        <LabeledInput
          ...
          name="note"
          label="Note"
          validation={{ required: true, maxLength: 64, pattern: /^[A-Za-z]+$/ }}
          error={errors.note}
        />
      </div>
      <LabeledSelect
      ...
      <span>
        {expires_in !== "0"
          ? `The token will expire on ${addDateStringToDate(
              expires_in,
              new Date()
            ).toDateString()}`
      ...
      {expires_in === "0" && (
        <div>
          <span>
            Webmaid strongly recommends that you set an expiration date for your
